...
published_at: 2026-05-24T02:00:48.549Z
crawled_at: 2026-05-24T02:00:48.552Z
word_count: 2943
reading_time_minutes: 15
tags: [gpt, multimodal, reasoning, safety, agents, infrastructure, api, product, open-source]
---
...
| --- | --- | --- |
| **Approach** | **Pros** | **Cons** |
| Unique IP:port per session (also known as native direct UDP) | Direct client-to-server media path<br>No forwarding layer in the data path | Requires one public UDP port per session<br>Large port ranges are difficult to expose and secure<br>Poor fit for Kubernetes and cloud load balancers |
| Unique IP:port per server | Much smaller public UDP footprint than per-session exposure<br>One shared socket per server can demultiplex many sessions | Works cleanly on a single host, but not across a shared load-balanced fleet by itself<br>Session demultiplexing on a single host only helps after a packet reaches that host; across a load-balanced fleet, the first packet can still land on the wrong instance, so you still need a deterministic way to steer each session to the process that owns it |
| TURN relay (protocol-terminating) | Clients only need to reach the TURN relay address and port<br>Can centralize policy at the edge | TURN allocations add setup round trips<br>Moving or recovering allocations across TURN servers is still difficult |
| Stateless forwarder + stateful terminator (OpenAI’s relay + transceiver) | Small public UDP footprint<br>Transceiver still owns the full WebRTC session | Adds one forwarding hop before media reaches the owning transceiver<br>Requires custom coordination between relay and transceiver |
...
\\
EngineeringMay 5, 2026](https://openai.com/index/mrc-supercomputer-networking/)



[An open-source spec for orchestration: Symphony\\
\\
EngineeringApr 27, 2026](https://openai.com/index/open-source-codex-orchestration-symphony/)